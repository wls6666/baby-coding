...
export class BabyCodingPanel implements vscode.WebviewViewProvider {
  ...
  private _getHtmlForWebview(_webview: vscode.Webview) {
    ...
    <body>
        ...
        <div class="actions">
            <button onclick="sendSystemMessage('setup')">⚙️ Setup</button>
            <button onclick="sendSystemMessage('startProject')">🚀 New Project</button>
        </div>
        ...
        <script>
            ...
            function renderPlan(plan) {
                ...
                plan.steps.forEach(step => {
                    ...
                    html += '<button class="run-btn" onclick="executeStep(\\'' + step.id + '\\')">▶ Run</button>';
                    html += '</div>';
